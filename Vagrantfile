...
Vagrant.configure('2') do |config|
  ...
  if vm_ansible
    ...
    ansible_groups = "#{base_folder}/groups/#{provision_groups}"
    ansible_playbook = "#{base_folder}/#{provision_action}.yml"
    raise "missign groups #{ansible_groups}" unless File.file?(ansible_groups)
    raise "missign playbook #{ansible_playbook}" unless File.file?(ansible_playbook)
